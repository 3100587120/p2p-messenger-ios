...
struct AccountNavigationButton: View {
    ...
    var body: some View {
        VStack {
            if let selectedAccount = getSelectedAccount() {
                Button(action: {
    ...
    private func getSelectedAccount() -> AccountViewModel? {
        guard let selectedAccountId = selectedAccountId, !selectedAccountId.isEmpty else {
            return nil
        }
        return viewModel.accountList.first(where: { $0.id == selectedAccountId })
    }
}
...
struct ConversationSection: View {
    ...
    private var filteredConversations: [ConversationViewModel] {
        ...
        let normalizedQuery = searchText.normalized()

        return conversations.filter { conversation in
            conversation.name.normalized().containsCaseInsensitive(string: normalizedQuery)
        }
    }
